...
export function ProductCard({ product }: ProductCardProps) {
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-lg hover:shadow-lg transition-shadow duration-200 overflow-hidden">
              ...
              {productImages.map((_, index) => (
                <button
                  ...
                  className={`w-2 h-2 rounded-full transition-colors ${
                    index === currentImageIndex ? 'bg-orange-500' : 'bg-white/60'
                  }`}
                />
              ))}
            </div>
      ...
      <div className="p-4">
        <h3 className="text-sm font-medium text-gray-800 mb-2 line-clamp-2 hover:text-orange-600 cursor-pointer">
          {product.name}
        </h3>
        ...
        <div className="flex items-center mb-2">
          ...
          <span className="text-xs text-blue-600 ml-2 hover:text-orange-600 cursor-pointer">(2,847)</span>
        </div>
